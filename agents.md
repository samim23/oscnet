...
## Equinox-Specific Practices and Rules (for use with JAX)

* **Module Design**:

  * Define custom layers and models using `equinox.Module`. This makes them compatible with JAX transformations and PyTree traversal.
  * Annotate constant/static fields with `static=True` to avoid issues with JIT compilation and gradient tracing.

* **State Management**:

  * Store trainable parameters and internal state (e.g., weights, biases) as fields inside an `equinox.Module`.
  * Use `equinox.filter_*` functions (`filter_grad`, `filter_value_and_grad`, `filter_jit`, etc.) to handle mixed trees of differentiable and non-differentiable parameters.

* **Randomness and Initialization**:

  * Accept `key: jax.random.PRNGKey` in your model/layer constructors to allow reproducible random initialization.
  * Never call `jax.random.split` inside the `__init__` method—pass split keys externally to retain functional purity.

* **Model Composition**:

  * Compose models like functions: `output = model(input)`.
  * Prefer many small, composable `Module` objects over monolithic implementations for better modularity, testing, and reuse.

* **Gradient Handling**:

  * Use `equinox.filter_value_and_grad` or `filter_grad` to compute gradients while excluding non-differentiable components.
  * Combine with `optax` for optimization using `equinox.apply_updates`.

* **Parameter Updates**:

  * Use `equinox.tree_at` for safe, functional updates of specific fields in a module (e.g., for freezing layers or applying masks).

* **Debugging and Inspection**:

  * Use `equinox.partition` to split model parameters into differentiable and static components.
  * Use `jax.tree_util.tree_map` or `tree_leaves` to explore or verify model structure.
  * For serialization, use `equinox.tree_serialise_leaves()` and `tree_deserialise_leaves()` to save/load model weights.

* **Common Pitfalls**:

  * Don’t forget to mark non-array values (e.g. `int`, `bool`) as `static=True` in Modules.
  * Avoid side-effects or logic inside `__init__` that depends on data — keep constructors pure and JIT-safe.
  * Be careful with model copying or caching — always treat `Module` objects as immutable, even if Python allows mutation.

* **Best Practices**:

  * Organize layers and models as reusable Modules in a `layers/` or `models/` directory.
  * Use Equinox + Optax + JAX as a canonical trio: Equinox for modeling, Optax for optimization, JAX for transformations.
